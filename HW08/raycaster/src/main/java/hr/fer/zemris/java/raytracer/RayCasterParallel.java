...
/**
 * Program RayCasterParallel demonstrates rendering 3D scenes using ray-tracing
 * algorithm and Phong lighting model with parallelization.
 * 
 * @author Filip Gulan
 * @version 1.0
 *
...
public class RayCasterParallel {
	...
	/**
	 * Start point of program RayCasterParallel.
	 * 
	 * @param args
	...
	private static IRayTracerProducer getIRayTracerProducer() {
		return new IRayTracerProducer() {
			...
			public void produce(Point3D eye, Point3D view, Point3D viewUp,
				...
				/**
				 * Thread task for recursively computing color for each pixel on
				 * scene.
				 * 
				 * @author Filip Gulan
				 * @version 1.0
				 *
				 */
				class Calculation extends RecursiveAction {

					/** Class serial version. */
					private static final long serialVersionUID = -4667897528899536048L;
					/** Threshold. */
					 ...
					 *            End point.
					 */
					public Calculation(int startY, int endY) {
						this.startY = startY;
						this.endY = endY;
					}

					/**
					 * Computes recursively color for each pixel on scene.
					 */
					@Override
					protected void compute() {
						if (this.endY - this.startY + 1 <= THRESHOLD) {
							computeDirect();
						} else {
							Calculation task1 = new Calculation(startY, endY
									- (endY - startY) / 2);
							Calculation task2 = new Calculation(endY
									- (endY - startY) / 2, endY);
					...
					/**
					 * Computes directly color for each pixel on scene.
					 */
				...
				pool.invoke(new Calculation(0, height));
				pool.shutdown();
