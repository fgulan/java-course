...
/**
 * ObjectMultistack is type of collection where stacks are
 * stored in a map. Each stack has its own unique name which
 * is also a key for the stack in a map. Also stack can hold 
...
public class ObjectMultistack {
	...
	/**
	 * Adds a new value to the given stack. If stack with given name
	 * does not exists, method creates one with given name.
	 ...
	 * @throws IllegalArgumentException If stack with given name does not exists.
	 */
	public ValueWrapper pop(String name) {
		if (stack.containsKey(name)) {
			...
			throw new IllegalArgumentException("There is no stack with given name!");
		}
	}
	 ...
	 * @param name Stack name.
	 * @return <code>true</code> if stack is empty, <code>false</code> othrewise.
	 */
	...
	/**
	 * MultistackEntry class represents stack object. It holds value
	...
	static class MultistackEntry {
		 ...
		 * @param nextEntry Reference to the next entry.
		 */
